# Aakarshika Priydarshi

**Lead Full-Stack Engineer · TypeScript/JavaScript · Cloud-Native Distributed Systems**

Full-stack engineer with 9+ years of experience designing and operating **production systems end-to-end** — from single-page web UIs to REST APIs, databases, and AWS cloud deployments. Proven track record building **scalable, multi-tenant platforms** and distributed data systems serving 50K+ users.

---

## Experience

### Senior Software Engineer — MdThink, Maryland  
*2022 — Present*  
*Stack: Node.js · TypeScript/JavaScript · Angular · React · PostgreSQL · AWS*

- Led **end-to-end architecture and development** of multi-tenant regulatory platforms serving **~50K users and ~5K daily requests**, spanning SPA frontend, REST API, database, and AWS cloud layers.

- Designed and owned **core platform modules** including: **Reporting** (on-demand + scheduled), **Notifications** system, **Real-time chat** (WebSockets), **RBAC authorization** layer, **Large file upload** system (S3 + streaming), **Compliance and inspection** workflows

- Architected and delivered a **reporting system using API-triggered and scheduled pipelines**, isolating heavy workloads from core services and improving system maintainability and developer velocity.

- Built **scalable REST APIs** using Node.js/JavaScript (LoopBack), handling complex business workflows and multi-tenant data models.

- Designed and optimized **Aurora RDS/PostgreSQL schemas and queries**, improving performance by ~20% through query restructuring, indexing, and aggregation optimization.

- Developed **modular single-page Angular frontend architecture** with reusable components, lazy loading, and performance optimizations, improving application startup time and maintainability.

- Implemented **JWT + session-based authentication and RBAC**, enforcing secure, role-based access across the platform.

- Integrated **real-time systems using WebSockets** for chat and large file upload workflows.

- Introduced **integration testing into CI/CD pipelines**, improving release stability and reducing regression issues.

- Deployed and operated applications on **AWS (Lambda, S3, EC2, IAM, Aurora RDS)**, collaborating with DevOps on containerized/ECS-style environments and deployment pipelines.

- Debugged and resolved **production issues across full stack layers**, including critical data consistency issues impacting audit and approval workflows.

- Led **technical decision-making, cloud software design, code reviews, and sprint planning**, mentoring engineers and guiding system design across teams.

---

### Full Stack Developer — MdThink  
*Apr 2019 — Aug 2022*

- Built full-stack features across **Angular web UIs, Node.js/JavaScript backend APIs, and PostgreSQL database layers** for enterprise regulatory systems.

- Designed APIs and data models supporting **complex workflows and high-volume transactional systems.**

- Improved system performance through **query optimization and backend refactoring.**

- Collaborated with product, QA, and DevOps teams to deliver **end-to-end features across frontend, backend, database, and cloud deployment workflows.**

---

### Backend Developer — Neuron Inc  
*Dec 2017 — Jan 2019*

- Developed backend systems for a **multi-tenant AI chatbot platform** using Django and PostgreSQL.

- Designed REST APIs for **messaging, session tracking, and analytics ingestion**, enabling real-time user interactions and high-volume conversational data flows.

- Implemented **JWT authentication and RBAC**, securing platform access across user roles.

- Improved API performance and reliability through **validation, rate limiting, and scalable design patterns.**

---

### Backend Engineer — Delhivery  
*Dec 2016 — Sep 2017*

- Engineered **TB-scale data pipelines** using Python, Java, and Spark on AWS EMR.

- Built and optimized **distributed Spark jobs** for large-scale data transformation (JSON → Parquet) and analytics ingestion into Amazon Redshift.

- Developed **streaming and batch pipelines** using Kinesis and queue-based architectures, applying event-driven patterns transferable to Kafka/RabbitMQ systems.

- Improved **pipeline reliability** by introducing fault-tolerant processing, eliminating costly full-batch restarts (~6 hours) from S3 dumps.